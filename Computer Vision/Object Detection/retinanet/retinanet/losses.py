...
def calc_iou(a, b):
    ...
    # a ∪ b
    ua = torch.unsqueeze(
        (a[:, 2] - a[:, 0]) * (a[:, 3] - a[:, 1]), dim=1) + area_b - iw * ih
...
# - (1 - p_t)**gamma * log(p_t); y=1时, p_t=\hat{p}; y≠1时, p_t=1-\hat{p_t}
class FocalLoss(nn.Module):
    # def __init__(self):

    def forward(self, classifications, regressions, anchors, annotations):
        alpha = 0.25  # loss weight
        gamma = 2.0
        batch_size = classifications.shape[0]
        classification_losses = []
        regression_losses = []

        anchor = anchors[0, :, :]  # get a anchor from batch, batch中图片大小相同，anchor维度都相同
        ...
        for j in range(batch_size):
            classification = classifications[j, :, :]  # (batch, num_anchor, num_class)
            regression = regressions[j, :, :]

            bbox_annotation = annotations[j, :, :]
            # 挑出类别不为-1的
            bbox_annotation = bbox_annotation[bbox_annotation[:, 4] != -1]
            # classification ∈ [1e-4, 1-1e-4], 取log时防止报错
            classification = torch.clamp(classification, 1e-4, 1.0 - 1e-4)

            if bbox_annotation.shape[0] == 0:  # y=0时, p_t = 1-\hat{p_t} (without bbox)
                if torch.cuda.is_available():
                    ...
                    cls_loss = focal_weight * bce  # final focal loss
                ...
                continue
            # 区分正负样本
            iou = calc_iou(anchors[0, :, :], bbox_annotation[:, :4])  # (num_anchors, num_annotations)
            iou_max, iou_argmax = torch.max(iou, dim=1)  # (num_anchors, 1)
            ...
            # 逐元素比较，小于0.4的取1，否则取0
            targets[torch.lt(iou_max, 0.4), :] = 0  # 背景类
            # 逐元素比较，大于等于0.5的取1，否则取0
            ...
            if torch.cuda.is_available():
                cls_loss = torch.where(
                    torch.ne(targets, -1.0), cls_loss,  # 不等于-1的地方取1
