...
class CocoDataset(Dataset):
    ...
    def load_image(self, image_index):
        ...
        path = os.path.join(
            self.root_dir, self.set_name, image_info['file_name'])
...
class CSVDataset(Dataset):
    ...
    def __init__(self, train_file, class_list, transform=None):
        ...
        try:
            ...
            raise(ValueError(
                'invalid CSV annotations file: {}: {}'.format(
                    self.train_file, e)))
    ...
    def _read_annotations(self, csv_reader, classes):
        ...
        for line, row in enumerate(csv_reader):
            ...
            try:
                ...
                raise(ValueError(
                    'line {}: format should be \'img_file,x1,y1,x2,y2,'
                    'class_name\' or \'img_file,,,,,\''.format(line)))
...
class Resizer(object):
    ...
    def __call__(self, sample, min_side=608, max_side=1024):
        ...
        new_image = np.zeros((rows + pad_w,
                              cols + pad_h, cns)).astype(np.float32)
...
class AspectRatioBasedSampler(Sampler):
    ...
    def __len__(self):
        if self.drop_last:
            ...
            return (len(
                self.data_source) + self.batch_size - 1) // self.batch_size
